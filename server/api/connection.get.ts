    ...
    for (const r of rows as any[]) {
      ...
      if (!map.has(key)) {
        map.set(key, {
          node_1: r.node_1,
          node_2: r.node_2,
          is_wheelchair_inaccessible: !Boolean(r.wheelchair_accessible),
          media: []
        })
      }
      if (r.media_id != null) {
        map.get(key).media.push({ media_id: r.media_id, media_type: r.media_type, media_url: r.media_url, order_num: r.order_num })
      }
    }
